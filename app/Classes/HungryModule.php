...
class HungryModule extends RequestGateway{
    ...
    public function cacheDataHome(){
        ...
        try{
            ...
            if (!Cache::has('cache_rest_section')){
                // die();
                $restSection = RequestGateway::get("/directories/home?coupon_type=promotions&limit=12", array());
                // echo '<pre>'. print_r($restSection,true).'</pre>'; die();
               if(isset($restSection['responseText']['status']) && $restSection['responseText']['status']!='error'){
                    ...
                    View::share('restSection',[]); // share var to all view (globle)
               }
            }
            else{
    ...
    // cauch data on second section top-category // home menu
    public function cacheDataHomeTopCate(){
        try{
            // Cache::forget('cache_top_cate_home');
            if (!Cache::has('cache_top_cate_home')){
                // die();
                $topCategoryHome = RequestGateway::get("/directories/home/menu?limit=1000", array());
    ...
    // cache data on category section
    public function orginPaymentParking(){

        try{

            // Cache::forget('orgin_payment_parking');
            if (!Cache::has('orgin_payment_parking')){
                // die();
                $orgin_payment_parking = RequestGateway::get("/dimensions?type=drink,feature,origin,payment_method,parking&ignore-offset=true&is_landing_page=1", array());
                // echo '<pre>'. print_r($orgin_payment_parking,true).'</pre>'; die();

                // echo '<pre>'. print_r($tmp->toArray(),true).'</pre>'; die();

               if(isset($orgin_payment_parking['responseText']['status']) && $orgin_payment_parking['responseText']['status']!='error'){

                    $collection = collect($orgin_payment_parking['responseText']['result']);

                    $tmp = $collection->groupBy('type'); // groupby type 


                    $orgin_payment_parking = $tmp->toArray();
                    // echo '<pre>'. print_r($orgin_payment_parking,true).'</pre>'; die();

                    if(sizeof($orgin_payment_parking)>0){
                        // store all category to cache in one day
                        $expiresAt = 1440;
                        Cache::put('orgin_payment_parking', $orgin_payment_parking, $expiresAt);
                    }
                    // echo '<pre>'. print_r($orgin_payment_parking,true).'</pre>'; die();
                    View::share('orgin_payment_parking',$orgin_payment_parking); // share var to all view (globle)

               }
               else{
                    View::share('orgin_payment_parking', []);
               }
            }
            else{

                // get data from cache
                $orgin_payment_parking = Cache::get('orgin_payment_parking');
                // echo '<pre>'. print_r($orgin_payment_parking,true).'</pre>'; die();
                // if something error when get data remove the cache and redirect to get data again
                if(sizeof($orgin_payment_parking)<=0){
                    Cache::forget('orgin_payment_parking');
                ...
                // echo '<pre>'. print_r($orgin_payment_parking['recomendations'],true).'</pre>'; die();
                View::share('orgin_payment_parking', $orgin_payment_parking); // share var to all view (globle)
            }
    ...
    public function getStarRate($restItem){
        ...
        $ranNum = rand(2, 10);
        $rateCount = isset($restItem['rating']) ? $restItem['rating'] : $ranNum;
